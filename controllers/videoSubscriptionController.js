...
export const createVideoOrder = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message:
        "Video subscription order was created.",
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Video payment verification failed."
      });
    }
    ...
    if (!pendingSubscription) {
      return res.status(404).json({
        success: false,
        message:
          "The video subscription order was not found."
      });
    }
    ...
    if (
      ...
      return res.status(200).json({
        success: true,
        message:
          "The video subscription is already active.",
        subscription:
          pendingSubscription,
        plan: selectedPlan
      });
    ...
    // Expire previous active video plan
    ...
    return res.status(500).json({
      success: false,
      message:
        "Unable to verify the video payment."
    });
  }
};

// Get current video subscription
export const getVideoSubscriptionStatus =
  async (req, res) => {
    try {
      const userId =
        req.headers[
      ...
      if (!selectedPlan) {
        return res.status(200).json({
          ...
          active: false,
          message:
            "The active video plan is invalid."
        });
      }
